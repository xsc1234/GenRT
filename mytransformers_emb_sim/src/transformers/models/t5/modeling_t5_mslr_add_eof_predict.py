...
from model.embedding_layer import Doc_embedding,Feature_embedding_pre_process,FC_SL
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        return outputs
#返回大小 [bs,input_seq]，表示在该状态下的输入文档排序分数
class T5_Att_Weight(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.q = nn.Linear(self.d_model, self.inner_dim, bias=False)
        self.k = nn.Linear(self.d_model, self.inner_dim, bias=False)
        self.v = nn.Linear(137, 137, bias=False)
        self.o = nn.Linear(self.inner_dim, self.d_model, bias=False)

        self.project_layer = nn.Linear(config.d_model, 137, bias=False)

        # self.project_layer1 = nn.Linear(1402, 700, bias=False)
        # self.silu1 = nn.SiLU(inplace=False)
        # self.project_layer2 = nn.Linear(700, 350, bias=False)
        # self.silu2 = nn.SiLU(inplace=False)
        self.FRB = FC_SL(feature_dim=274)
    ...
    def forward(
        ...
        #scores = (self.get_socre(self.FRB(self.silu2(self.project_layer2(self.silu1(self.project_layer1(matrix))))))).squeeze(2)#[bs,input_seq]
...
class T5Attention_Truncate(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.has_relative_attention_bias = has_relative_attention_bias
        ...
        self.v = nn.Linear(self.d_model, self.inner_dim, bias=False)
        self.o = nn.Linear(self.inner_dim, self.d_model, bias=False)
        self.binary_layer1 = nn.Linear(self.d_model,128)
        self.silu1 = nn.SiLU(inplace=False)
        self.binary_layer2 = nn.Linear(128,32)
        self.silu2 = nn.SiLU(inplace=False)
        self.binary_layer3 = nn.Linear(32,2)
        self.silu3 = nn.SiLU(inplace=False)
    ...
    def compute_bias_bi(self, query_length, key_length):
        """Compute binned relative position bias"""
        context_position = torch.arange(query_length, dtype=torch.long)[:, None]
        memory_position = torch.arange(key_length, dtype=torch.long)[None, :]
        relative_position = memory_position - context_position  # shape (query_length, key_length)
    ...
    def forward(
        ...
        if position_bias is None:
            if not self.has_relative_attention_bias:
                ...
                position_bias = self.compute_bias_bi(real_seq_length, key_length)
        ...
        scores += position_bias
        # scores += mask mask这一块还是会报错，暂时不加了
        attn_weights = nn.functional.softmax(scores.float(), dim=-1).type_as(
            scores
        )  # (batch_size, n_heads, seq_length, key_length)
        attn_weights = nn.functional.dropout(
        ...
        # print('max_idx')
        # print(max_idx)
        #print(attn_output[range(attn_output.shape[0]),max_idx].shape)
        b1 = self.silu1(self.binary_layer1(attn_output[range(attn_output.shape[0]),max_idx]))
        # print('b1')
        # print(b1)
        b2 = self.silu2(self.binary_layer2(b1))
        # print('b2')
        # print(b2)
        binary = self.silu3(self.binary_layer3(b2))
        #binary = self.binary_layer3(self.silu2(self.binary_layer2(self.silu1(self.binary_layer1(attn_output[range(attn_output.shape[0]),max_idx]))))) #[bs,input_seq,hidden]->[bs,hidden]
        # print('binary')
        # print(binary)
        return binary
...
class T5LayerSelfAttention_truncate(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = T5Attention_Truncate(config, has_relative_attention_bias=has_relative_attention_bias)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5_Attn_Weight_Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        if self.is_decoder:
            ...
            self.layer.append(T5LayerSelfAttention_truncate(config, has_relative_attention_bias=has_relative_attention_bias))
    ...
    def forward(
        ...
        attention_mask=None,
        frb_inputembeds=None,
        selected_embeds=None,
        selected_idx=None,
        position_bias=None,
        encoder_hidden_states=None,
        origin_hiddens_states=None,
        encoder_attention_mask=None,
        encoder_mask=None,
        encoder_decoder_position_bias=None,
        layer_head_mask=None,
        ...
        if do_cross_attention:
            ...
            # atten_wtight_outputs [bs,input_seq]  encoder_hidden_states [bs,input_seq,hidden_dim]
            score_logist = atten_wtight_outputs.clone()
            # print('1 st')
            if selected_embeds == []:
                selected_embeds_clone = []
            else:
                selected_embeds_clone = selected_embeds.clone()
            # for batch_idx in range(atten_wtight_outputs.shape[0]):
            #     print(atten_wtight_outputs[batch_idx])
            ranked_hidden_states = []
            max_idxs = []
            for batch_idx in range(atten_wtight_outputs.shape[0]):
                score_batch = score_logist[batch_idx]
                selected_idx_batch = selected_idx[batch_idx]
                if selected_embeds_clone == []:
                    selected_embeds_batch = []
                else:
                    selected_embeds_batch = selected_embeds_clone[batch_idx] #[input_seq_selected,hedden_dim]
                ranked_hidden_states_batch = origin_hiddens_states[batch_idx] #[input_seq,hedden_dim]
                idx_and_score = [(idx,score_batch[idx]) for idx in range(len(score_batch))]
                idx_and_score.sort(key=lambda x: (x[1], -x[0]), reverse=True) #[在输入的索引，预测分]
                if selected_embeds_batch == []:
                    max_idxs.append(0)
                else:
                    max_idxs.append(selected_embeds_batch.shape[0]) #在新构造的排序列表的位置
                count_last = 5 #超参数，控制选取当前状态后几步的item（包括当前状态）
                # print('selected_idx')
                # print(len(selected_idx_batch))
                # print(selected_idx_batch)
                # print('idx_and_score')
                # print(idx_and_score)
                # print(len(idx_and_score))
                # print('before')
                # print(max_idxs)
                for idx,_ in idx_and_score:
                    if(idx in selected_idx_batch): #如果某个batch内的query都比较短，比如最长是9，那么其他小于9的超过他本身
                        # 的长度之后，会重复选同一个mask处的idx，所有可以一直添加，需要对这个进行限制，而最长的那个
                        # 则不会重复，但是超过9后会一直continue，因为其他的idx都已经选过了，导致数组越界
                        continue
                    if selected_embeds_batch == []: #第一步，还没有选择任何item
                        selected_embeds_batch = ranked_hidden_states_batch[idx].unsqueeze(0)
                    else: #不是第一步，或者是第一步但是已经选择了item（向后扩展5步时）
                        if selected_embeds_batch.shape[0]+1 > len(idx_and_score):
                            #print(selected_embeds_batch.shape)
                            # print('we break')
                            # max_idxs[batch_idx] = max_idxs[batch_idx]-1
                            # for i in range(atten_wtight_outputs.shape[0]):
                            #     print(atten_wtight_outputs[i])
                            break
                        selected_embeds_batch = torch.cat([selected_embeds_batch,ranked_hidden_states_batch[idx].unsqueeze(0)],dim=0) #[input_seq_selected+1,hidden_dim]
                    count_last -= 1
                    if count_last == 0:
                        break
                if max_idxs[batch_idx] > len(idx_and_score)-1:
                    max_idxs[batch_idx] = len(idx_and_score)-1
                # print('after')
                # print(max_idxs)
                # print('selected')
                # print(selected_embeds_batch.shape)
                ranked_hidden_states.append(selected_embeds_batch.unsqueeze(0)) #[1,input_seq_selected+1,hidden_dim]
            ranked_hidden_states = torch.cat(ranked_hidden_states,dim=0) #[bs,input_seq_selected+1,hidden_dim]
            # print('ranked_hidden_states')
            # print(ranked_hidden_states.shape)
            ...
            # for batch_idx in range(atten_wtight_outputs.shape[0]):
            #     print(atten_wtight_outputs[batch_idx])
            return_atten_wtight_outputs = atten_wtight_outputs.clone()
            binary = self.layer[2](
                ranked_hidden_states,
                is_first_step=is_first_step,
                attention_mask=attention_mask,
            ...
            # print('rank_hidden_states')
            # print(ranked_hidden_states)
            # for batch_idx in range(ranked_hidden_states.shape[0]):
            #     print(ranked_hidden_states[batch_idx])
            # print('5 st')
            # print(atten_wtight_outputs)
            # for batch_idx in range(atten_wtight_outputs.shape[0]):
            #     print(atten_wtight_outputs[batch_idx])
        return return_atten_wtight_outputs,binary  # hidden-states, present_key_value_states, (self-attention position bias), (self-attention weights), (cross-attention position bias), (cross-attention weights)
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        if(self.is_decoder):
            ...
            model_list.append(T5_Attn_Weight_Block(config, has_relative_attention_bias=True))
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                if i == self.config.num_layers - 1 and self.is_decoder: ## decoder最后一层，直接返回注意力分数和二分类概率
                    layer_outputs,binary = layer_module(
                        hidden_states,
                        ...
                        origin_hiddens_states=origin_hiddens_states,
                    ...
                    # print('2 st')
                    # for batch_idx in range(layer_outputs.shape[0]):
                    #     print(layer_outputs[batch_idx])
                    return layer_outputs,binary
...
class T5ForRank_v2(T5PreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = encoder_outputs[0]
        origin_hiddens_states = hidden_states
        hidden_states = self.project_layer(hidden_states)
        hidden_states = (hidden_states+1).mul(input_frb_embeds) #[bs,len_input_seq,feature_dim]
        # print('hidden_states')
        ...
        frb_inputembeds = []
        selected_embeds = []
        all_step = 25 #训练时计算多少步
        op_step = 25 #优化前多少个item的loss
        beta = 1
        if(mode == 'train'):
            item_count = 0
            last_rank_idx = []
            ...
            while item_count < all_step:
                if item_count == 0:
                    is_first_step=True
                else:
                    is_first_step=False
                decoder_outputs,binary = self.decoder(
                    input_ids=None,
                    frb_inputembeds=frb_inputembeds,
                    selected_embeds=selected_embeds,
                    selected_idx=last_rank_idx,
                    is_first_step=is_first_step,
                    attention_mask=decoder_attention_mask,
                    inputs_embeds=decoder_inputs_embeds,
                    past_key_values=past_key_values,
                ...
                next_token_logits = decoder_outputs #[bs,input_seq]
                # print('3 st')
                # for batch_idx in range(next_token_logits.shape[0]):
                #     print(next_token_logits[batch_idx])
                new_embed = []
                new_frb_embed = []
                new_selected_embed = []
                #计算每个时间步的预测分布loss
                for batch_idx in range(decoder_outputs.shape[0]):
                    ...
                    next_token_logits_batch = next_token_logits[batch_idx].unsqueeze(0)
                    next_token_logits_batch = next_token_logits_batch.clone() #该条记录对应每个输入idx的分数[1,input_seq]
                    label = label.clone()
                    # print('label')
                    # print(label.shape)
                    # print('next_token_logits')
                    ...
                    ## 加速版 不设0
                    next_tokens_scores_batch = nn.functional.log_softmax(next_token_logits_batch.float(), dim=-1)
                    if item_count < op_step and train_mode == 0: #专注于优化前op_step个doc
                        label_soft = nn.functional.softmax(label.float(),dim=-1)
                        if item_count > 0:
                            loss_step = - (1 / torch.log(torch.from_numpy(np.array(item_count+1))))*torch.dot(next_tokens_scores_batch[0], label_soft[0])
                        else: #对第一个doc加大权重(乘10)
                            loss_step = - 10*torch.dot(next_tokens_scores_batch[0], label_soft[0])
                        # if batch_idx == 0:
                        #     print('step is {}'.format(item_count))
                        #     print('loss step is {}'.format(loss_step))
                    ...
                    new_selected_embed.append(origin_hiddens_states[batch_idx][next_token_batch[0].item()].unsqueeze(0))
                   ############## 计算截断二分类的loss###################
                    if train_mode == 1:
                        if next_token_batch[0].item() > len(x_doc_id_list[batch_idx])-1: #是mask处了
                            label_binary = torch.tensor([0.0, 1.0]).unsqueeze(0)
                        else:
                            # 截断的（前面已经选择的id+当前最大id）
                            batch_doc_id = x_doc_id_list[batch_idx]
                            batch_label = id_label[batch_idx]
                            batch_rank = copy.deepcopy(last_rank_idx[batch_idx])
                            ...
                            for idx in batch_rank:
                                try:
                                    ...
                                    continue
                            trunc_dcg_1 = get_truncate_dcg(pre, batch_label, all_step)
                            # 不截断的 （前面已经选择的id+当前最大id+按照顺序往后面看5个id)
                            score_batch = next_token_logits[batch_idx]
                            idx_and_score = [(idx, score_batch[idx]) for idx in range(len(score_batch))]
                            idx_and_score.sort(key=lambda x: (x[1], -x[0]), reverse=True)
                            count_last = 4  # 超参数，控制选取当前状态后几步的item
                            #该循环即往后选5(4+1)个id
                            for idx, _ in idx_and_score:
                                if (idx in batch_rank):
                                    continue
                                else:
                                    batch_rank.append(idx)
                                count_last -= 1
                                if count_last == 0:
                                    break
                            if (batch_label[0][1] == 0):
                            ...
                            for idx in batch_rank:
                                try:
                                    ...
                                    continue
                            trunc_dcg_0 = get_truncate_dcg(pre, batch_label, all_step)
                            # if trunc_dcg_0 > trunc_dcg_1:
                            #     label_binary = torch.tensor([1.0,0.0]).unsqueeze(0)
                            # else:
                            #     label_binary = torch.tensor([0.0, 1.0]).unsqueeze(0)
                            label_binary = torch.tensor([trunc_dcg_0,trunc_dcg_1]).unsqueeze(0)
                        #print(label_binary)
                        label_binary_soft = nn.functional.softmax(label_binary.float(), dim=-1)
                        logitst_binary = binary[batch_idx].unsqueeze(0)
                        #print(binary.shape)
                        logitst_binary_soft = nn.functional.log_softmax(logitst_binary.float(), dim=-1)
                        loss_binary = -1*torch.dot(label_binary_soft[0].to(logitst_binary_soft.device),logitst_binary_soft[0])
                        # if batch_idx == 0:
                        #     print('loss binary {}'.format(loss_binary))
                        loss_total = loss_total + loss_binary
                        loss_binary_total = loss_binary_total + loss_binary
                        ######################计算截断loss完毕############################
                new_embed = torch.cat(new_embed,dim=0).unsqueeze(1)
                new_frb_embed = torch.cat(new_frb_embed,dim=0).unsqueeze(1)
                new_selected_embed = torch.cat(new_selected_embed,dim=0).unsqueeze(1)
                decoder_inputs_embeds = torch.cat([decoder_inputs_embeds,new_embed],dim=1)
                if is_first_step:
                    frb_inputembeds = new_frb_embed
                    selected_embeds = new_selected_embed
                else:
                    frb_inputembeds = torch.cat([frb_inputembeds,new_frb_embed],dim=1)
                    selected_embeds = torch.cat([selected_embeds,new_selected_embed], dim=1)
                item_count += 1
            ## 计算step by step lambda loss
            if train_mode == 0:
                for batch_idx in range(decoder_inputs_embeds.shape[0]):
                    ...
                    for idx in batch_rank:
                        try:
                            ...
                            continue
                    ndcg = get_ndcg(pre,batch_label,all_step)
                    # print('pre:')
                    # print(ndcg)
                    # print(pre_seq)
                    # print(batch_label)
                    for rank_idx in range(min(len(batch_rank),len(batch_doc_id))):
                        if rank_idx == op_step: #专注于优化top10个doc
                            break
                        loss_lambda = 0
                        for post_rank_idx in range(rank_idx+1,min(len(batch_rank),len(batch_doc_id))):
                            ...
                            if batch_doc_id[rank_idx_of_input][3] == 0 and batch_doc_id[post_idx_of_input][3] > batch_doc_id[rank_idx_of_input][3]:
                                # print('label')
                                ...
                                #RankNet pairwise loss 参考Optimizing Dense Retrieval Model Training with Hard Negatives
                                ...
                                delta_ndcg = get_ndcg(pre_change,batch_label,all_step) - ndcg
                                # print('paiewise loss')
                                # print(paiewise_loss)
                                # print('delta_ndcg')
                                # print(delta_ndcg)
                                ...
                                #print(loss_now)
                                loss_total = loss_total + beta*loss_now
                                loss_lambda = loss_lambda + loss_now
                        # if batch_idx == 0:
                        #     print('loss lambda is {}'.format(loss_lambda))
            loss = loss_total / decoder_inputs_embeds.shape[0]
            #print('loss binary is {}'.format(loss_binary_total / decoder_inputs_embeds.shape[0]))
            return loss

        elif (mode == 'generate'):
            jieduan_idx = [all_step+10]*decoder_inputs_embeds.shape[0]
            item_count = 0
            last_rank_idx = []
            pre_score = []
            pre_score_before_soft = []
            step_rank_idx = []
            for i in range(decoder_inputs_embeds.shape[0]):
                last_rank_idx.append([])
                pre_score.append([])
                pre_score_before_soft.append([])
                step_rank_idx.append([])
            while item_count < 30: ##预测多少个item
                if item_count == 0:
                    is_first_step=True
                else:
                    is_first_step=False
                ...
                for batch_idx in range(decoder_outputs.shape[0]):
                    ...
                    # print('next_token_logits_batch')
                    # print(next_token_logits_batch.shape) torch.Size([1, 246])
                    next_token_logits_batch_list = [(idx,next_token_logits_batch[0,idx]) for idx in range(next_token_logits_batch[0].shape[0])]
                    next_token_logits_batch_list.sort(key=lambda x: (x[1], -x[0]), reverse=True)
                    next_token_logits_batch_list = [item[0] for item in next_token_logits_batch_list]
                    step_rank_idx[batch_idx].append(next_token_logits_batch_list)
                    if (not len(last_rank_idx[batch_idx]) == 0):
                        next_token_logits_batch[0,last_rank_idx[batch_idx]] = -100  # 上次选过的这次不选了
                    next_token_logits_batch[next_token_logits_batch[:,:] == -100] = -10000 # mask处为10000
                    pre_score_before_soft[batch_idx].append(next_token_logits_batch)
                    next_tokens_scores_batch = nn.functional.softmax(next_token_logits_batch.float(), dim=-1)
                    pre_score[batch_idx].append(next_tokens_scores_batch)
                    next_token_batch = torch.argmax(next_tokens_scores_batch, dim=-1)
                    last_rank_idx[batch_idx].append(next_token_batch[0].item())
                    # print('len last rand idx')
                    # print(len(last_rank_idx[batch_idx]))
                    ...
                    new_frb_embed.append(input_frb_embeds[batch_idx][next_token_batch[0].item()].unsqueeze(0))
                    new_selected_embed.append(origin_hiddens_states[batch_idx][next_token_batch[0].item()].unsqueeze(0))
                    logitst_binary = binary[batch_idx]
                    # print(logitst_binary)
                    # print(logitst_binary.shape)
                    jieduan = torch.argmax(logitst_binary, dim=-1).item()
                    # print(jieduan,item_count)
                    if jieduan == 1 and jieduan_idx[batch_idx] == all_step+10: #如果它没有被截断过，且需要截断
                        jieduan_idx[batch_idx] = item_count+1
                new_embed = torch.cat(new_embed,dim=0).unsqueeze(1)
                new_frb_embed = torch.cat(new_frb_embed,dim=0).unsqueeze(1)
                new_selected_embed = torch.cat(new_selected_embed,dim=0).unsqueeze(1)
                decoder_inputs_embeds = torch.cat([decoder_inputs_embeds,new_embed],dim=1)
                ...
                # print(decoder_inputs_embeds.shape) [batch_size,len_now,hidden_dim]每次加一
                item_count += 1
            # joblib.dump(pre_score_before_soft, '/data/xushicheng/seq2rank/out/pre_socre_before_soft_mslr1024')
            # # joblib.dump(pre_score,'/data/xushicheng/seq2rank/out/pre_socre_mslr1024')
            joblib.dump(labels,'/data/xushicheng/seq2rank/out_attn/mslr_label')
            joblib.dump(origin_hiddens_states,'/data/xushicheng/seq2rank/out_attn/mslr_embedding')
            return last_rank_idx,step_rank_idx
