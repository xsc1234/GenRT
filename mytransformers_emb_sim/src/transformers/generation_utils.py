...
class GenerationMixin:
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            next_token_logits = outputs.logits[:, -1, :] #最后一个单词的输出为该时间步预测的输出分布
    ...
    def greedy_search_for_rank(
        ...
        **model_kwargs,
        ...
        # keep track of which sequences are already finished
        unfinished_sequences = input_embeds.new(input_embeds.shape[0]).fill_(1)
        ...
        while True:
            ...
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
                # The following logic allows an early break if all peers finished generating their sequence
                this_peer_finished_flag = torch.tensor(0.0 if this_peer_finished else 1.0).to(input_embeds.device)
                # send 0.0 if we finished, 1.0 otherwise
                dist.all_reduce(this_peer_finished_flag, op=dist.ReduceOp.SUM)
                # did all peers finish? the reduced sum will be 0.0 then
                if this_peer_finished_flag.item() == 0.0:
            ...
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_embeds, **model_kwargs)
            ...
            next_token_logits = outputs[1][:, -1, :] #最后一个单词的输出为该时间步预测的输出分布
            ...
            input_embeds = torch.cat([input_embeds, next_tokens[:, None]], dim=1).to(model.device)
            #new_token_embeds =
            # input_embeds = []
            # for batch_idx in range(input_ids.shape[0]):
            #     input_embeds.append(model.transfer_layer(input_ids[batch_idx]).unsqueeze(0))
