...
import sys
import random
from model.modeling_t5_add_pos_eof_predict import t5_for_rank_v2
from torch.utils.data import DataLoader, Dataset
#from torch import nn
from data.load_dataset import Feature_dataset,collate_fn
from mytransformers_emb_sim.src.transformers.models.t5.modeling_t5_mslr_add_eof_predict import T5Config,T5ForRank_v2
...
class Trainer(object):
    def __init__(self, args):
        ...
        # self.dev_data_sub = self.dev_data[:1024]
        # joblib.dump(self.dev_data_sub,'./dataset/MSLR/FOLD1/processed_data/test_top40_sub.pkl')
        ...
        self.dev_loader = DataLoader(self.dev_set, batch_size=1024,collate_fn=collate_fn)
        ...
        self.model.load_state_dict(torch.load("./out_mslr_2_1/epoch_25.pth"))
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(self.dev_loader):
                count += len(batch[0])
                ndcg_1,ndcg_5,ndcg_10,step_ndcg = self.model(batch[0], batch[1], x_ts=batch[2], label_doc_id=batch[3], label_input_id=None,id_label=batch[5], score=None, input_embeds_matrix=batch[5], evaluate=True)
                sum_ndcg_1 += ndcg_1
                sum_ndcg_5 += ndcg_5
                sum_ndcg_10 += ndcg_10
                break
            print('NDCG@1 is {} NDCG@5 is {} NDCG@10 is {}'.format(sum_ndcg_1 / count,sum_ndcg_5 / count,sum_ndcg_10 / count))
